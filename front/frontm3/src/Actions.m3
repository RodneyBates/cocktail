 ...
 *)
 ...
 UNSAFE MODULE Actions;

IMPORT Word;
FROM ReuseIO         IMPORT WriteC, WriteS, WriteT, WriteI, WriteNl, tFile;
...
FROM WriteTok   IMPORT tLanguage, Language, SourceFileName;
  ...
  PROCEDURE PutAction (kind: tActionMode; a: tList; ap: PosType; c: tList; cp: PosType) =
    BEGIN
      WITH m2tom3_with_1=Action [kind] DO
        m2tom3_with_1.Act := a;
        m2tom3_with_1.ActPos := ap;
        m2tom3_with_1.Comm := c;
        m2tom3_with_1.CommPos := cp;
      END;
    END PutAction;

  PROCEDURE GetAction (kind: tActionMode; VAR a: tList; VAR ap: PosType; VAR c: tList; VAR cp: PosType) =
    BEGIN
      WITH m2tom3_with_2=Action [kind] DO
        a  := m2tom3_with_2.Act;
        ap := m2tom3_with_2.ActPos;
        c  := m2tom3_with_2.Comm;
        cp := m2tom3_with_2.CommPos;
      END;
    END GetAction;
  ...
  PROCEDURE WriteSemanticAction (f: tFile; a: tList; t: Word.T) =
  VAR
    c : CHAR;
    s : tString;
    i : tStringIndex;
    ...
    BEGIN
      ...
      WHILE NOT IsEmpty (a) DO
        GetString (LOOPHOLE (Head (a), tStringRef), s);
        Tail (a);
        i2 := Length (s);
        IF IsEmpty (a) THEN
    ...
    PROCEDURE WriteStringList (file: tFile; slist: tList) =
      BEGIN
        WHILE NOT IsEmpty (slist) DO
          WriteString (file, LOOPHOLE (Head (slist), tStringRef ));
          Tail (slist);
        END;
      END WriteStringList;
...
BEGIN
  dummyPos.Line := 0;
  dummyPos.Column := 0;
  FOR ActionMode := FIRST (tActionMode) TO LAST (tActionMode) DO
