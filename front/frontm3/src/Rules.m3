...
FROM FrontErrors	IMPORT eFatal, eError, eIdent, eString, eInternal, ErrorMessageI;
...
CONST eNoOperator = 41; eTermLeft = 42; eTokenNotDecl = 32;
    ...
    REVEAL
Expression = UNTRACED BRANDED REF  Node;

    TYPE
Node = 
      RECORD
	 Type: Operation; 
	          (* unitaere Operationen *)
	    Son:    Expression;
	     (* binaere Operationen *)
	    LeSon,
	    RiSon:  Expression; 
    ...
    REVEAL
MRules = UNTRACED BRANDED REF  Rule;
...
Rule = RECORD
  ...
  BEGIN
    HNode := Alloc(BYTESIZE(Node));
    IF HNode = NIL THEN
      ErrorT ("MakeLeafNode: Heap overflow");
    END;
    HNode^.Position := Pos;
    HNode^.Special  := NIL;
    voc := MakeVoc(sym,Pos);
    IF GetTokenType(voc) = TokenType.Term THEN
      HNode^.Type    := Operation.TermLeaf;
      HNode^.Token   :=	 voc;
    ELSE 
      HNode^.Type    := Operation.NonTermLeaf ;
      HNode^.Token   :=	 voc;
    END;
    RETURN HNode;
  END MakeLeafNode;
  ...
  BEGIN
    HNode := Alloc(BYTESIZE(Node));
    IF HNode = NIL THEN
      TextToString ("MakeActionNode : Heap overflow", s);
      ErrorMessageI(eInternal, eFatal, Pos, eString, ADR(s));
    END;
    HNode^.Special  := NIL;
    HNode^.Type := Operation.Action;
    HNode^.Act := Act;
    HNode^.Position := Pos;
    RETURN HNode;
  END MakeActionNode;
  ...
  BEGIN
    HNode := Alloc(BYTESIZE(Node));
    IF HNode = NIL THEN
      TextToString(" MakeUnaryNode : Heap overflow", s);
      ErrorMessageI (eInternal,eFatal, Pos, eString, ADR(s));
    END;
    HNode^.Special  := NIL;
    HNode^.Type	       := Type;
  ...
  BEGIN
    HNode := Alloc(BYTESIZE(Node));
    IF HNode = NIL THEN
      TextToString ("MakeBracketNode : Heap overflow", s);
      ErrorMessageI (eInternal,eFatal,Pos, eString, ADR (s));
    END;
    HNode^.Special  := NIL;
    HNode^.Type	       := Type;
  ...
  BEGIN
    ...
    HNode := Alloc(BYTESIZE(Node));
    IF HNode = NIL THEN
      TextToString ("MakeBinaryNode : Heap overflow", s);
      ErrorMessageI(eInternal,eFatal, Pos, eString, ADR (s));
    END;
    HNode^.Special  := NIL;
    HNode^.Type	       := Type;
    ...
    HNode^.LeSon := LSon;
    HNode^.RiSon := RSon;
    IF Type = Operation.Alternative THEN
      HNode^.HasPrio := FALSE;
    END;
    RETURN HNode;
  END MakeBinaryNode;
  ...
  BEGIN
    HNode := Alloc(BYTESIZE(Node));
    IF HNode = NIL THEN
      TextToString ("MakePrioAlternativeNode : Heap overflow", s);
      ErrorMessageI(eInternal,eFatal,Pos, eString, ADR(s));
    END;
    ...
    HNode^.Special  := NIL;
    HNode^.Type	       := Operation.Alternative;
    HNode^.Position   := Pos;
    HNode^.LeSon := LSon;
    HNode^.RiSon := RSon;
    RETURN HNode;
  ...
  BEGIN
    IF (Expr = NoExpression) OR (GetNodeOperation (Expr) # Operation.ArtAlternative) THEN
      Expr := MakeArtificialNode (Pos, Pos2, Expr, New);
    ELSE
      expr := Expr;
      LOOP
	last := expr^.RiSon;
	IF GetNodeOperation (last) # Operation.ArtAlternative THEN EXIT END;
	expr := last;
      END;
  ...
  BEGIN
    HNode := Alloc(BYTESIZE(Node));
    IF HNode = NIL THEN
      TextToString ("MakeArtificialNode : Heap overflow", s);
      ErrorMessageI(eInternal,eFatal,Pos,eString, ADR(s));
    END;
    HNode^.Special  := NIL;
    HNode^.Type	       := Operation.ArtAlternative;
    HNode^.Position   := Pos;
    HNode^.SecondPos  := SecPos;
    HNode^.LeSon := LSon;
    HNode^.RiSon := RSon;
  ...
  BEGIN
    IF Expr # NIL THEN
      Expr^.Special := Spec;
    ELSE
      ErrorT ("PutNodeSpecial : You tried to access an empty node");
    END;
  END PutNodeSpecial;
  ...
  BEGIN
    ...
    IF GetTokenType(Leftvoc) = TokenType.Term THEN
    ...
    ELSE
      ...
      HRule := Alloc(BYTESIZE(Rule));
      IF HRule = NIL THEN
	TextToString("MakeRule : Heap overflow", s);
	...
	IF Error # TokenError.NoError THEN
  ...
  BEGIN
    IF (GetNodeOperation(Expr) # Operation.TermLeaf) AND 
       (GetNodeOperation(Expr) # Operation.NonTermLeaf) THEN
      ErrorT ("GetLeafNode : Wrong Node Type");
    END;
    IF Expr # NIL THEN
      Voc := Expr^.Token;
      Pos := Expr^.Position;
    ELSE
      ErrorT ("GetLeafNode : Node empty");
    END;
  END GetLeafNode;
  ...
  BEGIN
    IF GetNodeOperation(Expr) # Operation.Action THEN
      ErrorT ("GetActionNode : Wrong Node Type");
    END;
    IF Expr # NIL THEN
      Act := Expr^.Act;
      Pos := Expr^.Position;
    ELSE
      ErrorT ("GetActionNode : Node empty");
    END;
  END GetActionNode;
  ...
  BEGIN
    IF (GetNodeOperation(Expr) # Operation.Star) AND 
       (GetNodeOperation(Expr) # Operation.Plus) THEN
      ErrorT ("GetUnaryNode : Wrong Node Type");
    END ;
    IF Expr # NIL THEN
      Pos := Expr^.Position;
      Son := Expr^.Son;
    ELSE
      ErrorT ("GetUnaryNode : Node empty");
    END;
  END GetUnaryNode;
  ...
  BEGIN
    IF (GetNodeOperation(Expr) # Operation.Bracket) AND 
       (GetNodeOperation(Expr) # Operation.Optional) THEN
      ErrorT ("GetBracketNode : Wrong Node Type");
    END ;
    IF Expr # NIL THEN
      Pos := Expr^.Position;
      ...
      Son := Expr^.Son;
    ELSE
      ErrorT ("GetBracketNode : Node empty");
    END;
  END GetBracketNode;
  ...
  BEGIN
    IF (GetNodeOperation(Expr) # Operation.Sequence) AND 
       (GetNodeOperation(Expr) # Operation.Separator) AND
       (GetNodeOperation(Expr) # Operation.Alternative) AND
       (GetNodeOperation(Expr) # Operation.ArtAlternative) THEN
      ErrorT ("GetBinaryNode : Wrong Node Type");
    END ;
    IF Expr # NIL THEN
      Pos  := Expr^.Position;
      LSon := Expr^.LeSon;
      RSon := Expr^.RiSon;
    ELSE
      ErrorT ("GetBinaryNode : Node empty");
    END; 
  END GetBinaryNode;
  ...
  BEGIN
    IF (GetNodeOperation(Expr) # Operation.Alternative) THEN
      ErrorT ("GetPrioAlternativeNode : Wrong Node Type");
    END ;
    IF Expr # NIL THEN
      Pos  := Expr^.Position;
      LSon := Expr^.LeSon;
    ...
    ELSE
      ErrorT ("GetPrioAlternativeNode : Node empty");
    END; 
  ...
  BEGIN
    IF (GetNodeOperation(Expr) # Operation.ArtAlternative) THEN
      ErrorT ("GetArtificialNode : Wrong Node Type");
    END ;
    IF Expr # NIL THEN
      Pos  := Expr^.Position;
      SecPos := Expr^.SecondPos;
      ...
      RSon := Expr^.RiSon;
    ELSE
      ErrorT ("GetArtificialNode : Node empty");
    END; 
  ...
  BEGIN
    IF Expr # NIL THEN
      RETURN Expr^.Special;
    ELSE
      ErrorT ("GetNodeSpecial : Node empty");
      RETURN NIL;
    END
  END GetNodeSpecial;
  ...
  BEGIN
    IF NOT OpenForReading THEN
      ErrorT ("GetRule : You must not read here");
    END;
    IF RMRule = NIL THEN
  ...
  BEGIN
    IF NOT OpenForReading THEN
      ErrorT ("GetEssentialRule : You must not read here");
    END;
    IF RMRule = NIL THEN
      (* Am Ende der Liste angelangt *)
      RETURN FALSE
...
(* TODO: This or very similar local procedure occurs in several modules. *) 
