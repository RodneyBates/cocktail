...
PROCEDURE Message  (ErrorText: ARRAY OF CHAR; ErrorClass: CARDINAL; Position: tPosition);
   ...
   BEGIN
      ...
      HandleMessage 
        (FALSE, VAL ( CARDINAL , MakeIdent (String) )
...
PROCEDURE StringMessage  
  (ErrorText: tString; ErrorClass: CARDINAL; Position: tPosition);
   ...
   BEGIN
      HandleMessage 
        (FALSE, VAL ( CARDINAL , MakeIdent ( ErrorText ) )  
        , ErrorClass, Position, None, NIL
        );
   END StringMessage ;
...
PROCEDURE WriteInfo (InfoClass: CARDINAL; Info: ADDRESS);
   ...
   BEGIN
      ...
      | Short   : PtrToShort    := Info; WriteI (Out, VAL ( INTEGER , PtrToShort^), 0);
      | Long    : PtrToLong     := Info; WriteI (Out, VAL ( INTEGER , PtrToLong^), 0);
...
PROCEDURE WriteMessage  (IsErrorCode: BOOLEAN; ErrorCode, ErrorClass: CARDINAL;
   ...
   BEGIN
      ...
      IF IsErrorCode THEN
         WriteCode (ErrorCode);
      ELSE
         WriteIdent (Out, VAL ( tIdent , ErrorCode ) );
      END;
   ...
   BEGIN
      ...
      FOR i := 1 TO MessageCount DO
         WITH ErrorTable [i] DO
            ...
            END;
            WriteMessage 
              ( IsErrorCode, VAL ( CARDINAL , ErrorCode )
              , VAL ( CARDINAL , ErrorClass ) , Position
              , VAL ( CARDINAL , InfoClass ) , Info
              );
         END;
      END;
      Out := StdError;
   ...
   BEGIN
      IF MessageCount < MaxError THEN
         ...
         WITH ErrorTable [MessageCount] DO
            ...
            ErrorNumber := VAL ( SHORTCARD , MessageCount )     ;
            ErrorCode   := VAL ( SHORTCARD , pErrorCode )       ;
            ErrorClass  := VAL ( SHORTCARD , pErrorClass )      ;
            InfoClass   := VAL ( SHORTCARD , pInfoClass )       ;
            CASE InfoClass OF
            | Integer   : PtrToInteger  := pInfo; vInteger      := PtrToInteger ^;
            | Short     : PtrToShort    := pInfo; vShort        := VAL ( INTEGER , PtrToShort   ^);
            | Long      : PtrToLong     := pInfo; vLong         := VAL ( INTEGER , PtrToLong    ^);
