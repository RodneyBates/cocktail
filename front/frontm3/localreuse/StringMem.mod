...
TYPE Memory		= ARRAY LONGINT [0 .. 65535] OF CHAR;
...
PROCEDURE GetString (r: tStringRef; VAR s: tString);
   VAR i	: tStringIndex;
   BEGIN
      s.Length := VAL ( tStringIndex , Length (r) );
      INC (r, 2);
      FOR i := 1 TO s.Length DO
...
PROCEDURE WriteStringMemory;
   ...
   BEGIN
	 ...
	 WriteI (StdOutput, VAL ( INTEGER , StringPtr ) , 5);
	 WriteC (StdOutput, ' ');
	 WriteString (StdOutput, StringPtr);
	 WriteNl (StdOutput);
	 sLength 
           := VAL ( LONGINT , Length (StringPtr) + 2 ); (* damned MODULA *)
