...
TYPE
   RowType	= POINTER TO ARRAY CHAR OF DStateRange;
   DStateInfo	= RECORD
		     Row	: RowType	;
		     Semantics	: tSet		;
		     Default	: DStateRange	;
...
PROCEDURE MakeDState	(): DStateRange;
   ...
   BEGIN
	 ...
	 FOR Ch := 0C TO LastCh DO Row^[Ch] := DNoState; END;
...
PROCEDURE PutTable	(State: DStateRange; Ch: CHAR; NextState: DStateRange);
   BEGIN
	 ...
	 ELSE						(* Exclude *)
	    IF (Ch = FirstElmt) AND (Ch < LastCh) THEN INC (FirstElmt); END;
...
PROCEDURE ComputeAmbiguousStates;
   ...
   BEGIN
      ...
      MakeArray (PredCount2Ptr, PredCount2Size, TSIZE (SHORTCARD));
	 ...
	 FOR Successor := 1 TO DStateCount DO
      ...
      ReleaseArray (PredCount2Ptr, PredCount2Size, TSIZE (SHORTCARD));
