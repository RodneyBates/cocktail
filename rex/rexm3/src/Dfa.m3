...
PROCEDURE MakeDState	(): DStateRange =
   ...
   BEGIN
	 ...
	 ExtendArray (TablePtr, TableSize, BYTESIZE (DStateInfo));
      END;
      WITH m2tom3_with_5=TablePtr^[DStateCount] DO
         RowSize	:= ORD (LastCh) + 1;
	 MakeArray (m2tom3_with_5.Row, RowSize, BYTESIZE (DStateRange));
...
PROCEDURE MinimizeDfa() =
   ...
   BEGIN
      ...
      MakeArray (DBPtr, DBSize, BYTESIZE (DBElmt));
...
PROCEDURE ComputeSuccGraph() =
   ...
   BEGIN
      ...
      MakeArray (SuccSetPtr, SuccSetSize, BYTESIZE (tSet));
      FOR State := 1 TO DStateCount DO
...
PROCEDURE ComputeAmbiguousStates() =
   ...
   BEGIN
      ...
      MakeArray (PredCount2Ptr, PredCount2Size, BYTESIZE (SHORTCARD));
...
PROCEDURE ComputeCyclicStates() =
   ...
   BEGIN
      ...
      MakeArray (SuccPtr, SuccSize, BYTESIZE (tSet));
      FOR State := 1 TO DStateCount DO
...
BEGIN
   ...
   MakeArray (TablePtr, TableSize, BYTESIZE (DStateInfo));
END Dfa.
