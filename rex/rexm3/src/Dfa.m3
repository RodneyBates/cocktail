...
TYPE
   RowType      = UNTRACED BRANDED REF  ARRAY CHAR OF DStateRange;
   DStateInfo   = RECORD
                     Row        : RowType       ;
                     Semantics  : tSet          ;
                     Default    : DStateRange   ;
...
PROCEDURE ReleaseDState (State: DStateRange) =
   ...
   BEGIN
      DEC (DStateCount);
      RowSize := ORD (LastCh) + 1;
      ReleaseArray (TablePtr^[State].Row, RowSize, BYTESIZE (DStateRange));
...
PROCEDURE ComputeSuccGraph() =
   ...
   BEGIN
      ...
      MakeArray (LOOPHOLE(SuccSetPtr,ADDRESS), SuccSetSize, BYTESIZE (tSet));
      FOR State := 1 TO DStateCount DO
...
PROCEDURE ComputeAmbiguousStates() =
   ...
   BEGIN
      ...
      MakeArray (LOOPHOLE(PredCountPtr,ADDRESS), PredCountSize, BYTESIZE (SHORTCARD));
      PredCount2Size    := DStateCount + 1;
      MakeArray (LOOPHOLE(PredCount2Ptr,ADDRESS), PredCount2Size, BYTESIZE (SHORTCARD));
...
PROCEDURE ComputeCyclicStates() =
   ...
   BEGIN
      ...
      MakeArray (LOOPHOLE(SuccPtr,ADDRESS), SuccSize, BYTESIZE (tSet));
      FOR State := 1 TO DStateCount DO
...
BEGIN
   ...
   MakeArray (LOOPHOLE(TablePtr,ADDRESS), TableSize, BYTESIZE (DStateInfo));
END Dfa.
