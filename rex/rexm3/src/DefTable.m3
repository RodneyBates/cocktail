...
; VAR DefTablePtr : UNTRACED BRANDED REF DefTable 
...
; PROCEDURE MakeIdentDef 
  ...
  = BEGIN (* MakeIdentDef *) 
      INC ( DefCount ) 
    ; IF DefCount = DefTableSize 
      THEN 
        ExtendArray 
          ( LOOPHOLE ( DefTablePtr , ADDRESS ) 
          , DefTableSize 
          , BYTESIZE ( DefType ) 
          ) 
      END (* IF *) 
    ; WITH With_1 = DefTablePtr ^ [ DefCount ] 
      DO With_1 . Symbol := pSymbol 
...
; PROCEDURE MakeStartDef ( pSymbol : tIdent ; pNumber : SHORTCARD ) 
  ...
  = BEGIN (* MakeStartDef *) 
    ...
    ; IF DefCount = DefTableSize 
      ...
      END (* IF *) 
    ; WITH With_2 = DefTablePtr ^ [ DefCount ] 
      DO With_2 . Symbol := pSymbol 
...
; PROCEDURE GetDef ( pSymbol : tIdent ) : DefRange 
  ...
  = BEGIN (* GetDef *) 
      FOR Def := 1 TO DefCount 
      DO IF DefTablePtr ^ [ Def ] . Symbol = pSymbol 
         THEN 
           RETURN Def 
         END (* IF *) 
      END (* FOR *) 
...
; PROCEDURE GetIdentDef 
  ...
  = BEGIN (* GetIdentDef *) 
      WITH With_3 = DefTablePtr ^ [ pDef ] 
      DO pSymbol := With_3 . Symbol 
      ; pTree := With_3 . Tree 
