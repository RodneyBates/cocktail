...
TYPE
   BufferDescriptor	= RECORD
	 Buffer		: POINTER TO ARRAY SHORTINT [0 .. BufferSize] OF CHAR;
...
PROCEDURE FillBuffer	(f: tFile);
   BEGIN
	 ...
	 BufferIndex := 0;
	 BytesRead 
           := VAL ( SHORTINT , System.Read (f, ADR (Buffer^ [1]), BufferSize) );
	 IF BytesRead <= 0 THEN
	    BytesRead := 0;
	    EndOfFile := TRUE;
	 END;
...
PROCEDURE Read		(f: tFile; Buffer: ADDRESS; Size: CARDINAL): INTEGER;
   VAR						(* binary		*)
      BufferPtr : POINTER TO ARRAY CARDINAL [0 .. 100000000] OF CHAR;
      i		: CARDINAL;
   BEGIN
      BufferPtr := Buffer;
      WITH BufferPool [f] DO
...
PROCEDURE ReadR		(f: tFile): REAL;	(* real     number	*)
   ...
   BEGIN
	 ...
	 IF Mantissa <= MaxIntDiv10 THEN
	    ...
	    IF Mantissa 
               <= VAL ( LONGCARD , MaxInt - (ORD (ch) - ORD ('0')) ) THEN
...
PROCEDURE ReadShort	(f: tFile): SHORTINT;	(* shortint number	*)
   BEGIN
      RETURN VAL ( SHORTINT , ReadI (f) );
   END ReadShort;

PROCEDURE ReadLong	(f: tFile): LONGINT;	(* longint  number	*)
   BEGIN
      RETURN VAL ( LONGINT , ReadI (f) );
   END ReadLong;
...
PROCEDURE WriteFlush	(f: tFile);		(* flush output buffer	*)
   BEGIN
	 ...
	 BytesRead 
           := VAL ( SHORTINT
                  , System.Write 
                      (f, ADR (Buffer^ [1]), VAL ( INTEGER , BufferIndex ) )
                  );
	 BufferIndex := 0;
      END;
   END WriteFlush;
...
PROCEDURE WriteR	(f: tFile; n: REAL; Before, After, Exp: CARDINAL);
   ...
   BEGIN
	 ...
	 IntegerDigits := Exponent;
      ...
      REPEAT
	 DEC (FirstDigit);
	 Digits [FirstDigit] := VAL ( CARDINAL , Mantissa MOD 10 ) ;
	 ...
	 ELSE
	    WriteC (f, '+');
	 END;
	 WriteN (f, VAL ( LONGCARD , Exponent ) , Exp - 1, 10);
      END;
   END WriteR;
...
PROCEDURE WriteN	(f: tFile; n: LONGCARD; FieldWidth, Base: CARDINAL);
   ...
   BEGIN
	 ...
	 INC (length);
	 digits [length] 
           := MyCHR [ VAL ( INTEGER , n MOD VAL ( LONGCARD , Base ) ) ];
	 n := n DIV VAL ( LONGCARD , Base );
      UNTIL n = 0;
...
PROCEDURE WriteShort	(f: tFile; n: SHORTINT; FieldWidth: CARDINAL);
   BEGIN					(* shortint number	*)
      WriteI (f, VAL ( INTEGER , n ) , FieldWidth);
   END WriteShort;
