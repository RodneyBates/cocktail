...
VAR
   TablePtr		: POINTER TO 
                          ARRAY tIdent [0 .. MAX(tIdent)] OF IdentTableEntry;
   ...
   BEGIN
      WITH s DO					(* hash *)
  	 IF Length = 0 THEN
	    HashTableIndex := 0;
	 ELSE 
	    HashTableIndex := ( ORD (Chars [1]) 
                                + ORD (Chars [Length]) * 11
			        + VAL ( CARDINAL , Length )* 26
                              ) 
                              MOD HashTableSize;
	 END;
      END;
	 ...
	 END;
      ...
      THEN
          ...
          IF lIdentCount = VAL ( tIdent , IdentTableSize ) THEN
...
PROCEDURE WriteIdents;
   ...
   BEGIN
	 ...
	 WriteI (StdOutput, VAL ( INTEGER , i ) , 5);
	 WriteC (StdOutput, ' ');
...
PROCEDURE WriteHashTable;
   ...
   BEGIN
	 ...
	 WriteNl (StdOutput);
      ...
      WriteI (StdOutput, VAL ( INTEGER , IdentCount ) , 5);
      WriteNl (StdOutput);
   END WriteHashTable;
