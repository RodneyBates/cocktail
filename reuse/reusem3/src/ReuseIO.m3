...
PROCEDURE ReadOpenT (FileNameT: TEXT): tFile =
   ...
   BEGIN
      ...
      WITH WBuffPool=BufferPool [f] DO
         ...
         WBuffPool.BytesRead        := 0;
         WBuffPool.OpenForOutput    := FALSE;
         WBuffPool.EndOfFile        := FALSE;
      END;
      CheckFlushLine (f);
      RETURN f;
   END ReadOpenT;

(* TODO: Rename this ReadOpenA. *) 
PROCEDURE ReadOpen (READONLY FileNameA: ARRAY OF CHAR): tFile =
   (* open input file     *)
   VAR   FileNameT: TEXT; 
   BEGIN
     FileNameT := Text.FromChars (FileNameA);
     RETURN ReadOpenT (FileNameT);
   END ReadOpen;
...
PROCEDURE WriteOpenT (FileNameT: TEXT): tFile =
   ...
   BEGIN
      ...
      WITH WBuffPool=BufferPool [f] DO
         WBuffPool.Buffer          := Alloc (BufferSize + 1);
         WBuffPool.BufferIndex     := 0;
         WBuffPool.OpenForOutput   := TRUE;
      END;
      CheckFlushLine (f);
      RETURN f;
   END WriteOpenT;

(* TODO: Rename this WriteOpenA. *) 
PROCEDURE WriteOpen (READONLY FileNameA: ARRAY OF CHAR): tFile =
   (* open  output file    *)
   VAR FileNameT: TEXT; 
   BEGIN
      FileNameT := Text.FromChars (FileNameA);
      RETURN WriteOpenT (FileNameT); 
   END WriteOpen;
