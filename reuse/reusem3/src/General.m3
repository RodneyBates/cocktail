...
; VAR ForAlign : RECORD char : CHAR ; longreal : LONGREAL END (* RECORD *) 
  ; argc : INTEGER 
...
; PROCEDURE Log2 ( x : M2LONGINT ) : Word . T 
  ...
  ; BEGIN (* Log2 *) 
      y := 0 
    ; IF x >= 65536 THEN INC ( y , 16 ) ; x := x DIV 65536 END (* IF *) 
    ; IF x >= 256 THEN INC ( y , 8 ) ; x := x DIV 256 END (* IF *) 
    ; IF x >= 16 THEN INC ( y , 4 ) ; x := x DIV 16 END (* IF *) 
    ; IF x >= 4 THEN INC ( y , 2 ) ; x := x DIV 4 END (* IF *) 
    ; IF x >= 2 THEN INC ( y , 1 ) ; x := x DIV 2 END (* IF *) 
    ; RETURN y 
    END Log2 
...
; PROCEDURE AntiLog ( x : M2LONGINT ) : Word . T 
  ...
  ; BEGIN (* AntiLog *) 
    ...
    ; IF ( x MOD 65536 ) = 0 
      ...
      ; x := x DIV 65536 
      END (* IF *) 
    ; IF ( x MOD 256 ) = 0 THEN INC ( y , 8 ) ; x := x DIV 256 END (* IF *) 
    ; IF ( x MOD 16 ) = 0 THEN INC ( y , 4 ) ; x := x DIV 16 END (* IF *) 
    ; IF ( x MOD 4 ) = 0 THEN INC ( y , 2 ) ; x := x DIV 4 END (* IF *) 
    ; IF ( x MOD 2 ) = 0 THEN INC ( y , 1 ) ; x := x DIV 2 END (* IF *) 
    ; RETURN y 
    END AntiLog 
...
; PROCEDURE Exp2 ( x : Word . T ) : M2LONGINT 
  ...
  ; BEGIN (* Exp2 *) 
      y := 1 
    ; IF x >= 16 THEN DEC ( x , 16 ) ; y := y * 65536 END (* IF *) 
    ; IF x >= 8 THEN DEC ( x , 8 ) ; y := y * 256 END (* IF *) 
    ; IF x >= 4 THEN DEC ( x , 4 ) ; y := y * 16 END (* IF *) 
    ; IF x >= 2 THEN DEC ( x , 2 ) ; y := y * 4 END (* IF *) 
    ; IF x >= 1 THEN DEC ( x , 1 ) ; y := y * 2 END (* IF *) 
    ; RETURN y 
    END Exp2 
...
; PROCEDURE Exp10 ( x : INTEGER ) : REAL 
  ...
  ; BEGIN (* Exp10 *) 
   ...
   (* IF x >= 32 THEN DEC (x, 32); y := y * 1.0E32; END; PCS10 compatibility *) 
    ...
    ; IF x >= 16 THEN DEC ( x , 16 ) ; y := y * 1.0E16 END (* IF *) 
    ; IF x >= 16 THEN DEC ( x , 16 ) ; y := y * 1.0E16 END (* IF *) 
    ; IF x >= 8 THEN DEC ( x , 8 ) ; y := y * 1.0E8 END (* IF *) 
    ; IF x >= 4 THEN DEC ( x , 4 ) ; y := y * 1.0E4 END (* IF *) 
    ; IF x >= 2 THEN DEC ( x , 2 ) ; y := y * 1.0E2 END (* IF *) 
    ; IF x >= 1 THEN DEC ( x , 1 ) ; y := y * 1.0E1 END (* IF *) 
