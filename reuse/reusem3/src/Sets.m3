...
PROCEDURE ReleaseSet (VAR Set: tSet) =
   VAR BitsetCount : LONGINT;
   BEGIN
      BitsetCount := VAL (   Set.LastBitset + 1,LONGINT );
...
PROCEDURE Include (VAR Set: tSet; Elmt: tElement) =
   BEGIN
	 ...
	 INCL (m2tom3_with_33.BitsetPtr^[VAL (   Elmt DIV BitsPerBitset,SHORTCARD ) ]
              , Elmt MOD BitsPerBitset
              );
	 m2tom3_with_33.Card      := VAL (   NoCard,tInternalElmt );
...
PROCEDURE Exclude (VAR Set: tSet; Elmt: tElement) =
   BEGIN
	 ...
	 EXCL ( m2tom3_with_34.BitsetPtr^[ VAL (   Elmt DIV BitsPerBitset,SHORTCARD ) ]
              , Elmt MOD BitsPerBitset
              );
