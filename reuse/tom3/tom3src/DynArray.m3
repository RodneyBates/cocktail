 ...
 * RMB 93/10/11 Added two type conversions of (General.)MaxAlign to LONGINT
...
PROCEDURE MakeArray    (VAR ArrayPtr	: ADDRESS	;
   ...
   BEGIN
      ...
      | 1, 3=> INC (ElmtCount, BYTESIZE (LONGINT) - 1 - ((ElmtCount - 1) MOD BYTESIZE (LONGINT)));
      END;
...
PROCEDURE ExtendArray  (VAR ArrayPtr	: ADDRESS	;
   ...
   VAR
      NewPtr		: ADDRESS;
      Source, Target	: UNTRACED BRANDED REF  LONGINT;
      i			: LONGINT;
   BEGIN
      ElmtSize := AlignedSize (ElmtSize);
	 ...
	 Source := ArrayPtr;
	 Target := NewPtr;
	 FOR i := 1 TO ElmtCount * ElmtSize DIV BYTESIZE (LONGINT) DO
	    Target ^ := Source ^;
	    Source := LOOPHOLE (LOOPHOLE (Source,ADDRESS) + BYTESIZE (LONGINT),ADDRESS);
	    Target := LOOPHOLE (LOOPHOLE (Target,ADDRESS) + BYTESIZE (LONGINT),ADDRESS);
	 END;
...
PROCEDURE AlignedSize  (ElmtSize: LONGINT): LONGINT =
   VAR Align	: LONGINT;
   BEGIN
      IF ElmtSize >= VAL (   MaxAlign,LONGINT ) THEN
	 Align := VAL (   MaxAlign,LONGINT );
      ELSE
