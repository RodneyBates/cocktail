...
PROCEDURE ArrayToString	(a: ARRAY OF CHAR; VAR s: tString);
   ...
   BEGIN
      i := 0;
      LOOP
	 IF a [ VAL ( CARDINAL , i ) ] = 0C THEN EXIT; END;
	 s.Chars [i+1] := a [ VAL ( CARDINAL , i ) ];
	 INC (i);
...
PROCEDURE StringToArray	(VAR s: tString; VAR a: ARRAY OF CHAR);
   ...
   BEGIN
	 ...
	 a [ VAL ( CARDINAL , i-1 ) ] := s.Chars [i];
      END;
      a [ VAL ( CARDINAL , s.Length ) ] := 0C;
   END StringToArray;
	...
	'e' : Mantissa := 1;
	 ...
	 IF Mantissa <= MaxIntDiv10 THEN
	    ...
	    IF Mantissa 
	       ...
	       INC (Mantissa, VAL ( LONGCARD , ORD (ch) - ORD ('0') ) );
	    ELSE
	       INC (TruncatedDigits);
	    END;
	 ELSE
