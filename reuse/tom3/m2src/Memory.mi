...
BEGIN
   ByteCount 
     := LONGINT 
          (BITSET (ByteCount + VAL ( LONGINT , MaxAlign ) - 1) 
           * AlignMasks [MaxAlign]
          );
   ...
   IF ByteCount <= MaxSizeSmallBlock THEN	(* handle small block *)
      ...
      IF SmallChain [ VAL ( tSmallBlockRange , ByteCount ) ] # NIL 
      THEN	(* obtain block from freelist *)
	 BlockPtr := SmallChain [ VAL ( tSmallBlockRange , ByteCount ) ];
	 SmallChain [ VAL ( tSmallBlockRange , ByteCount ) ] 
           := BlockPtr^.Successor;
	 RETURN BlockPtr;
   ...
   ELSE						(* handle large block *)
	 ...
	 INC (MemoryUsed, VAL ( LONGCARD , ByteCount ) ); 
	 RETURN BlockPtr;
      END;
   END;
END Alloc;
