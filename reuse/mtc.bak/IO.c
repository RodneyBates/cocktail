...
{
  {
    ...
    if (W_1->FlushLine) {
      ...
      IO_WriteFlush(IO_StdError);
    }
    W_1->BufferIndex = 0;
    W_1->BytesRead = VAL(SHORTINT, Read(f, ADR(W_1->Buffer->A[1]), BufferSize));
    if (W_1->BytesRead <= 0) {
      W_1->BytesRead = 0;
      W_1->EndOfFile = TRUE;
    }
...
# endif
{
  ...
  f = OpenInput(FileName, O_1);
  {
    ...
    W_2->Buffer = (struct S_2 *)Memory_Alloc(BufferSize + 1);
    W_2->BufferIndex = 0;
    W_2->BytesRead = 0;
...
# endif
{
  Close(f);
  {
    ...
    Memory_Free(BufferSize + 1, W_3->Buffer);
    W_3->Buffer = NIL;
  }
}
...
# endif
{
  ...
  while ('0' <= ch && ch <= '9') {
    if (Mantissa <= MaxIntDiv10) {
      ...
      if (Mantissa <= VAL(LONGCARD, MaxInt - (ORD(ch) - ORD('0')))) {
...
# endif
{
  return VAL(SHORTINT, IO_ReadI(f));
}
...
# endif
{
  ...
  COPY_OPEN_ARRAY(FileName, O_3, CHAR)
  f = OpenOutput(FileName, O_3);
  {
    register BufferDescriptor *W_8 = &BufferPool.A[f - -1];

    W_8->Buffer = (struct S_2 *)Memory_Alloc(BufferSize + 1);
    W_8->BufferIndex = 0;
...
# endif
{
  ...
  Close(f);
  {
    ...
    Memory_Free(BufferSize + 1, W_9->Buffer);
    W_9->Buffer = NIL;
  }
}
...
{
  {
    ...
    W_10->BytesRead = VAL(SHORTINT, Write(f, ADR(W_10->Buffer->A[1]), VAL(INTEGER, W_10->BufferIndex)));
    W_10->BufferIndex = 0;
  }
}
...
# endif
{
  ...
  do {
    DEC(FirstDigit);
    Digits.A[FirstDigit] = VAL(CARDINAL, Mantissa % 10);
  ...
  if (Exp > 0) {
    ...
    IO_WriteN(f, VAL(LONGCARD, Exponent), Exp - 1, 10);
  }
}
...
# endif
{
  ...
  do {
    INC(length);
    digits.A[length] = MyCHR.A[VAL(INTEGER, n % VAL(LONGCARD, Base))];
    n = n / VAL(LONGCARD, Base);
  } while (!(n == 0));
  {
...
# endif
{
  IO_WriteI(f, VAL(INTEGER, n), FieldWidth);
}
...
void IO__init()
{
  ...
  if (!has_been_called) {
    ...
    BufferPool.A[IO_StdInput - -1].Buffer = (struct S_2 *)Memory_Alloc(BufferSize + 1);
    BufferPool.A[IO_StdOutput - -1].Buffer = (struct S_2 *)Memory_Alloc(BufferSize + 1);
    BufferPool.A[IO_StdError - -1].Buffer = (struct S_2 *)Memory_Alloc(BufferSize + 1);
    ...
    CheckFlushLine(IO_StdInput);
    CheckFlushLine(IO_StdOutput);
    CheckFlushLine(IO_StdError);
  }
}
