...
# endif
{
  IO_WriteS(IO_StdError, "string too long, max. 255", 25L);
...
# endif
{
  ...
  {
    Strings_tStringIndex B_7 = 1, B_8 = General_MinSHORTCARD(s1->Length, s2->Length);
...
# endif
{
  ...
  i = 0;
  for (;;) {
    if (a[VAL(CARDINAL, i)] == '\0') {
      goto EXIT_1;
    }
    s->Chars.A[i + 1] = a[VAL(CARDINAL, i)];
    INC(i);
    if (i > (O_1 - 1)) {
      goto EXIT_1;
    }
...
# endif
{
...

  {
    ...
    if (B_11 <= B_12)
      for (i = B_11;; i += 1) {
        a[VAL(CARDINAL, i - 1)] = s->Chars.A[i];
        if (i >= B_12) break;
      }
  }
  a[VAL(CARDINAL, s->Length)] = '\0';
}
  ...
  while ('0' <= ch && ch <= '9') {
    if (Mantissa <= MaxIntDiv10) {
      ...
      if (Mantissa <= VAL(LONGCARD, MaxInt - (ORD(ch) - ORD('0')))) {
        INC1(Mantissa, VAL(LONGCARD, ORD(ch) - ORD('0')));
      } else {
        INC(TruncatedDigits);
      }
    } else {
