...
# endif
{
  ...
  *ArrayPtr = Memory_Alloc(*ElmtCount * ElmtSize);
  if (*ArrayPtr == NIL) {
    IO_WriteS(IO_StdError, "MakeArray: out of memory", 24L);
...
# endif
{
  ...
  if (NewPtr == NIL) {
    IO_WriteS(IO_StdError, "ExtendArray: out of memory", 26L);
    IO_WriteNl(IO_StdError);
  } else {
...
# endif
{
  ...
  if (ElmtSize >= VAL(LONGINT, General_MaxAlign)) {
    Align = VAL(LONGINT, General_MaxAlign);
  } else {
