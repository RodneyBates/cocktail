...
/* RMB 10-93 Removed main program stuff. (Its work is now generated
   in line by mtc in the program module's translation.)
   This is for compatibility with WRL Modula-2. */
...
# endif
{
    return 
      ( lo <= hi 
        ? ~ (CARDINAL)0X0L >> lo << lo + SYSTEM_MaxSet - hi >> SYSTEM_MaxSet - hi 
        : 0X0L
      );
}
...
/* RMB These duplicate declarations generated by WRL's parameters.mod.
       If a mixture of WRL and mtc generated objects are linked, 
       hopefully, the common mechanism will create only one copy. */

int    parameters_argc;
char **parameters_argv;
char **parameters_envp;
