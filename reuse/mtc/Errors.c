...
# endif
{
  (*HandleMessage)(TRUE, ErrorCode, ErrorClass, Position, Errors_None, NIL);
}
...
# endif
{
  ...
  (*HandleMessage)(FALSE, VAL(CARDINAL, Idents_MakeIdent(&String)), ErrorClass, Position, Errors_None, NIL);
  FREE_OPEN_ARRAYS
}
...
# endif
{
  ...
  (*HandleMessage)(FALSE, VAL(CARDINAL, Idents_MakeIdent(&String)), ErrorClass, Position, InfoClass, Info);
  FREE_OPEN_ARRAYS
}

void Errors_StringMessage
# ifdef __STDC__
(Strings_tString ErrorText, CARDINAL ErrorClass, Positions_tPosition Position)
# else
(ErrorText, ErrorClass, Position)
Strings_tString ErrorText;
CARDINAL ErrorClass;
Positions_tPosition Position;
# endif
{
  (*HandleMessage)(FALSE, VAL(CARDINAL, Idents_MakeIdent(&ErrorText)), ErrorClass, Position, Errors_None, NIL);
}
...
# endif
{
  ...
  switch (ErrorClass) {
  case Errors_Fatal:;
    IO_WriteS(Out, "Fatal       ", 12L);
    break;
  case Errors_Restriction:;
    IO_WriteS(Out, "Restriction ", 12L);
    break;
  case Errors_Error:;
    IO_WriteS(Out, "Error       ", 12L);
    break;
  case Errors_Warning:;
    IO_WriteS(Out, "Warning     ", 12L);
    break;
  case Errors_Repair:;
    IO_WriteS(Out, "Repair      ", 12L);
    break;
  case Errors_Note:;
    IO_WriteS(Out, "Note        ", 12L);
    break;
  case Errors_Information:;
    IO_WriteS(Out, "Information ", 12L);
    break;
  default :
    IO_WriteS(Out, "Error class: ", 13L);
    IO_WriteI(Out, ErrorClass, 0);
    break;
  }
}
...
# endif
{
  ...
  case Errors_SyntaxError:;
    IO_WriteS(Out, "syntax error", 12L);
    break;
  case Errors_ExpectedTokens:;
    IO_WriteS(Out, "expected tokens", 15L);
    break;
  case Errors_RestartPoint:;
    IO_WriteS(Out, "restart point", 13L);
    break;
  case Errors_TokenInserted:;
    IO_WriteS(Out, "token inserted ", 15L);
    break;
  case Errors_WrongParseTable:;
    IO_WriteS(Out, "parse table mismatch", 20L);
    break;
  case Errors_OpenParseTable:;
    IO_WriteS(Out, "cannot open parse table", 23L);
    break;
  case Errors_ReadParseTable:;
    IO_WriteS(Out, "cannot read parse table", 23L);
    break;
  case Errors_TooManyErrors:;
    IO_WriteS(Out, "too many errors", 15L);
    break;
  default :
    IO_WriteS(Out, " error code: ", 13L);
    IO_WriteI(Out, ErrorCode, 0);
    break;
  }
}
...
# endif
{
  ...
  case Errors_Integer:;
    ...
    IO_WriteI(Out, *PtrToInteger, 0);
    break;
  case Errors_Short:;
    PtrToShort = (SHORTCARD *)Info;
    IO_WriteI(Out, VAL(INTEGER, *PtrToShort), 0);
    break;
  case Errors_Long:;
    PtrToLong = (LONGINT *)Info;
    IO_WriteI(Out, VAL(INTEGER, *PtrToLong), 0);
    break;
  case Errors_Real:;
    PtrToReal = (REAL *)Info;
    IO_WriteR(Out, *PtrToReal, 1, 10, 1);
    break;
  case Errors_Boolean:;
...
# endif
{
  ...
  if (IsErrorCode) {
    ...
    Idents_WriteIdent(Out, VAL(Idents_tIdent, ErrorCode));
  }
...
# endif
{
  ...
  Sort_Sort(1, MessageCount, IsLess, Swap);
  Out = File;
  {
      ...
      for (i = B_1;; i += 1) {
        {
          ...
          WriteMessage(W_1->IsErrorCode, VAL(CARDINAL, W_1->ErrorCode), VAL(CARDINAL, W_1->ErrorClass), W_1->Position, VAL(CARDINAL, W_1->InfoClass), Info);
        }
        if (i >= B_2) break;
      }
  }
...
# endif
{
  ...
  if (MessageCount < MaxError) {
    INC(MessageCount);
    {
      ...
      W_2->ErrorNumber = VAL(SHORTCARD, MessageCount);
      W_2->ErrorCode = VAL(SHORTCARD, pErrorCode);
      W_2->ErrorClass = VAL(SHORTCARD, pErrorClass);
      W_2->InfoClass = VAL(SHORTCARD, pInfoClass);
      ...
      case Errors_Short:;
        ...
        W_2->U_1.V_3.vShort = VAL(INTEGER, *PtrToShort);
        break;
      case Errors_Long:;
        PtrToLong = (LONGINT *)pInfo;
        W_2->U_1.V_4.vLong = VAL(INTEGER, *PtrToLong);
        break;
      case Errors_Real:;
      ...
      case Errors_Set:;
        ...
        W_2->U_1.V_10.vSet = (Sets_tSet *)Memory_Alloc(sizeof(Sets_tSet));
  ...
  if (pErrorClass == Errors_Fatal) {
    Errors_WriteMessages(IO_StdError);
    (*Errors_Exit)();
  }
}
