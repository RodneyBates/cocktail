...
# endif
{
  ...
  NeededSpace = VAL(LONGINT, s->Length) + 2;
  while (MemorySpaceLeft < NeededSpace) {
    OldMemorySize = MemorySize;
    DynArray_ExtendArray(&MemoryPtr, &MemorySize, sizeof(CHAR));
...
# endif
{
  ...
  s->Length = VAL(Strings_tStringIndex, StringMem_Length(r));
  INC1(r, 2);
  {
...
# endif
{
  ...
  while (StringPtr < MemoryFreePtr) {
    IO_WriteI(IO_StdOutput, VAL(INTEGER, StringPtr), 5);
    IO_WriteC(IO_StdOutput, ' ');
    StringMem_WriteString(IO_StdOutput, StringPtr);
    IO_WriteNl(IO_StdOutput);
    sLength = VAL(LONGINT, StringMem_Length(StringPtr) + 2);
    INC1(StringPtr, sLength);
  }
  IO_WriteNl(IO_StdOutput);
  IO_WriteI(IO_StdOutput, VAL(INTEGER, StringPtr), 5);
  IO_WriteS(IO_StdOutput, " Bytes", 6L);
  IO_WriteNl(IO_StdOutput);
}
...
void StringMem__init()
{
  ...
  if (!has_been_called) {
    ...
    DynArray_MakeArray(&MemoryPtr, &MemorySize, sizeof(CHAR));
