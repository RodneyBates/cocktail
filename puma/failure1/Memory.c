...
# endif
{
  ...
  if (ByteCount <= MaxSizeSmallBlock) {
    ...
    if (SmallChain.A[VAL(tSmallBlockRange, ByteCount) - 4] != NIL) {
      BlockPtr = (tBlockPtr)SmallChain.A[VAL(tSmallBlockRange, ByteCount) - 4];
      SmallChain.A[VAL(tSmallBlockRange, ByteCount) - 4] = (ADDRESS)BlockPtr->Successor;
      ...
      if ((LONGINT)(PoolEndPtr - (LONGCARD)PoolFreePtr) < ByteCount) {
        ...
        PoolFreePtr = Memory_Alloc(PoolSize);
    ...
    if (ByteCount < PoolSize) {
      ...
      INC1(Memory_MemoryUsed, VAL(LONGCARD, ByteCount));
      return (ADDRESS)BlockPtr;
    }
  }
}
...
# endif
{
  ...
  if (ByteCount <= MaxSizeSmallBlock) {
    ...
    BlockPtr->Successor = (tBlockPtr)SmallChain.A[VAL(tSmallBlockRange, ByteCount) - 4];
    SmallChain.A[VAL(tSmallBlockRange, ByteCount) - 4] = (ADDRESS)BlockPtr;
  } else {
    ChainNumber = General_Log2(ByteCount);
